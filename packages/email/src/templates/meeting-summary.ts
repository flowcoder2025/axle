...
export function meetingSummaryEmail(props: MeetingSummaryEmailProps): string {
  ...
  const actionList = props.actionItems
    .map((item, idx) => `<li style="margin: 8px 0;"><strong>${idx + 1}.</strong> ${escapeHtml(item)}</li>`)
    .join("");
...
<div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto; color: #1a1a1a;">
  <div style="background: #f8f9fa; padding: 32px; border-radius: 8px;">
    ...
    <p style="margin: 0 0 24px; font-size: 14px; color: #6b7280;">${escapeHtml(props.date)}</p>

    <h3 style="margin: 0 0 8px; font-size: 16px;">${escapeHtml(props.meetingTitle)}</h3>
