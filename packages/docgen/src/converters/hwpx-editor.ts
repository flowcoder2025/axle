...
/**
 * Collect all text runs (<hp:t> or <t> elements) inside a node and concatenate.
 * HWPX uses the `hp` namespace; normalise by also checking bare element names.
 */
function collectText(node: unknown): string {
  ...
  if (typeof node === "object") {
    ...
    for (const [key, val] of Object.entries(obj)) {
      ...
      // Text run elements in HWPX: hp:t  / hsp:t / t
 ...
 *
 * HWPX is a ZIP archive containing XML section files. This function:
 * 1. Extracts the ZIP
