...
preflight() {
  ...
  # v3.1: Graph RAG Memory 초기화 (비차단)
  if [[ "${GRAPH_MEMORY_ENABLED:-false}" == "true" ]]; then
    if bash "$SCRIPTS_DIR/memory-init.sh" 2>/dev/null; then
      log "Graph RAG Memory initialized ($GRAPH_DB_PATH)"
      # 루프 시작 시 decay 업데이트
      source "$SCRIPTS_DIR/memory-helpers.sh" 2>/dev/null || true
      memory_update_decay 2>/dev/null || true
    else
      log "Graph RAG Memory init failed — continuing without graph memory"
      export GRAPH_MEMORY_ENABLED=false
    fi
  fi
...
${vault_content}"
  ...
  # 8. v3.1: Graph RAG Memory (구조화된 관계 기반 컨텍스트)
  if [[ "${GRAPH_MEMORY_ENABLED:-false}" == "true" && -n "$wi_name" ]]; then
    local graph_results
    local wi_intent="GENERAL"
    [[ "$wi_name" =~ -feat ]] && wi_intent="NEW_FEATURE"
    [[ "$wi_name" =~ -fix ]] && wi_intent="BUG_FIX"
    [[ "$wi_name" =~ -refactor ]] && wi_intent="REFACTOR"

    graph_results=$(bash "$SCRIPTS_DIR/memory-query.sh" "$wi_name" "$VAULT_PROJECT_NAME" "$wi_intent" 2>/dev/null)
    local graph_count
    graph_count=$(echo "$graph_results" | jq 'length' 2>/dev/null || echo "0")

    if [[ "$graph_count" -gt 0 ]]; then
      local graph_context=""
      while IFS= read -r entry; do
        [[ -z "$entry" || "$entry" == "null" ]] && continue
        local gtype gname gsummary
        gtype=$(echo "$entry" | jq -r '.type // ""' 2>/dev/null)
        gname=$(echo "$entry" | jq -r '.name // ""' 2>/dev/null)
        gsummary=$(echo "$entry" | jq -r '.summary // ""' 2>/dev/null)
        [[ -n "$gname" ]] && graph_context+="- [${gtype}] ${gname}: ${gsummary}
"
      done <<< "$(echo "$graph_results" | jq -c '.[]' 2>/dev/null)"

      if [[ -n "$graph_context" ]]; then
        parts+="[GRAPH MEMORY — 관련 entity (${wi_intent}, ${graph_count}건)]
${graph_context}"
      fi
    fi
  fi
