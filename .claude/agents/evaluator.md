...
- 채점 기반 판정: 0~10점. 임계치(9.0) 미만이면 되돌린다.
...
### 4. 판정
- **9.0 이상**: PASS → 채점표를 리드에게 반환. **리드가** `mkdir -p .flowset/eval-results && touch .flowset/eval-results/WI-{NNN}.pass` 마커를 생성한다. evaluator는 마커를 만들지 않는다.
- **9.0 미만**: FAIL → 채점표를 리드에게 반환. 리드가 해당 팀원에게 ISSUES 전달 → 수정 → 리드가 다시 evaluator spawn
