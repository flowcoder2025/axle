...
### [2026-05-12] flowset.sh v3.0.1 패치 — 가짜 완료 차단 게이트 (root cause fix)
3차 재발(2026-05-12 WI-622~626) 근본 원인 봉합. flowset.sh 패치 + 글로벌 템플릿 동기화.

**Root cause** (2026-05-12 #3 사고):
- WI-621 worker가 `feature/WI-621-...` 브랜치에서 작업 → PR #168 머지 → `safe_sync_main`은 브랜치 HEAD만 origin/main 으로 reset, **브랜치를 main으로 checkout하지 않음**
- WI-622 iteration 시작: `git branch --show-current`가 여전히 `feature/WI-621-...` 반환 → worker_branch != main → `wait_for_merge` 실행
- `wait_for_merge`가 `gh pr list --head feature/WI-621-... --state merged` 조회 → 이전 PR #168을 찾아 "이미 머지됨" 반환 0
- 워커가 PR을 안 만들었어도 `merge_result=0` → `mark_wi_done` 호출 → SSOT 오염

**Fix** (flowset.sh diff):
1. **`verify_wi_actually_merged` 헬퍼 추가**: `last_git_sha..HEAD` 범위에 WI prefix 커밋 존재 여부 검증
2. **`block_fake_completion` 헬퍼 추가**: 차단 시 guardrails 자동 기록
3. **순차 모드 mark_wi_done 게이트화**: 브랜치 경로(line 1962) + main 경로(line 1976) 모두 `verify_wi_actually_merged` 통과 시에만 호출
4. **iteration 끝마다 `git checkout main` 강제**: stale 브랜치 상속 차단
5. **`pre_iter_sha` 캡쳐**: iteration 시작 시점의 SHA를 verify의 baseline으로 사용
6. **FLOWSET_VERSION 3.0.0 → 3.0.1**

**검증**: `.flowset/scripts/test-verify-wi.sh` 4 시나리오 통과 (empty range / matching / unrelated / none baseline).

**Follow-up**:
- 글로벌 템플릿 `~/.claude/templates/flowset/flowset.sh` 동기화 필요 (이 PR 머지 후 수동 복사)
- 병렬 모드(worktree 경로) mark_wi_done 호출 (line ~1563/1614)에도 동일 게이트 적용 검토 — 현재는 워크트리당 1 WI 고정이라 위험 낮음
