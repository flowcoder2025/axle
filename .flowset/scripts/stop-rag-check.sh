...
# 6. Graph RAG Memory extraction + index update (v3.1, non-blocking)
if [[ "${GRAPH_MEMORY_ENABLED:-false}" == "true" ]]; then
  SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
  echo "$INPUT" | bash "$SCRIPT_DIR/memory-extract.sh" 2>/dev/null || true
  # Update vault index (source vault-helpers for vault_generate_index)
  if [[ "${VAULT_ENABLED:-false}" == "true" ]]; then
    source "$SCRIPT_DIR/vault-helpers.sh" 2>/dev/null || true
    vault_generate_index "${VAULT_PROJECT_NAME:-$PROJECT_NAME}" 2>/dev/null || true
  fi
fi
